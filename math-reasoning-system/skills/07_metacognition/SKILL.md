...
3. **Example Review:** Check whether existing examples support, fail to illuminate, or refute the current plan. If no examples have been examined and the problem is not purely formal, consider an example check.
4. **Bottleneck Identification:** If stuck, formulate the exact obstacle as a precise mathematical question.
5. **Problem-Solving Next Step Decision:** In `PROBLEM_SOLVING`, output exactly one of the following commands:
   ...
   - `EXAMPLE_CHECK`: Run `skills/12_example_counterexample/SKILL.md` to clarify a doubtful lemma, sharp hypothesis, or possible counterexample.
   - `ABANDON`: All plans exhausted. Trigger the generation of `artifacts/05_failure_report.md`.
6. **Research Next Step Decision:** In `EXPLORATORY_RESEARCH`, output exactly one of the following commands:
